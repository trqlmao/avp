...
lifecycle against a running server so you can watch every operation happen end to end. Node built-ins
only; the source is one file, [`src/client.ts`](src/client.ts).

```sh
npm install
npm start          # drives the flow against http://localhost:8787
...
```sh
AVP_SERVER_URL=http://vault.example:8787 npm start
```
...
1. **Generate keypairs** — each member is a fresh Ed25519 keypair (`node:crypto`). The base64 raw
